...
*   **Oyun Başlığı (Header) ve Araç Çubuğu:** `<GameHeader />` iki satırlı bir yapıdan oluşmalıdır. İlk satırda "Geri Dön", "Oyun Başlığı", "Tema Değiştirici" ve "Kullanıcı Profil/Avatar" butonları bulunur. İkinci satırda (Araç Çubuğu / Toolbar) ise varsa süre ölçer (timer) ve diğer oyun aksiyon ikonları (Nasıl Oynanır, Ayarlar, İstatistik, Paylaş, Joker vb.) yan yana dizilir. Kullanıcı deneyimini yavaşlatan **Hamburger Menü yapısı kesinlikle kullanılmaz**, tüm önemli aksiyonlar alt araç çubuğunda ekrana sığacak şekilde listelenmeli, dar ekranlarda taşmayı ve yana kaydırmayı (horizontal scroll) önlemek için **flex-wrap** ile alt satıra geçebilecek şekilde esnek (responsive) olarak görünür olmalıdır.
